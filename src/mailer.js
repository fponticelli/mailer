...
async function main() {
    ...
    const response = await prompt([
        {
            type: 'input',
            name: 'file',
            message: 'What is the name of the html file you want to mail?'
        },
        {
            type: 'input',
            name: 'destinations',
            message: 'What are the destination addresses?',
            initial: 'david@sobral.com'
        }
    ])
    
    // reads contents of the html file and saves it as a string
    const htmlString = readFileSync(`./build/${response.file}.html`, 'utf-8')
    ...
    let info = await gmailTransporter.sendMail({
        ...
        to: response.destinations, // list of receivers
